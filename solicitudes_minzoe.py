...
# ══════════════════════════════════════════════════════════════════════════════
                ...
                with rep:
                    ...
                    if servicio_ot == "Aires Acondicionados":
                            ...
                            with c_g1:
                                if st.button("✅ Enviar a revisión", type="primary",
                                    ...
                                    if not _foto_firma_h:
                                        ...
                                        # Generar HTML de fotos (obligatorias + extras con etiquetas)
                                        _cat_fin  = st.session_state.get(f"hvac_cat_{id_ot_sel}", "split")
                                        _prev_fin = st.session_state.get(f"r_prev_{id_ot_sel}", False)
                                        _FOTOS_KEYS_FIN = {
                                            "split": [
                                                ("placa_manj","Foto placa manejadora"),("placa_cond","Foto placa condensadora"),
                                                ("volt_manj","Voltaje manejadora"),("amp_manj","Amperaje manejadora"),
                                                ("volt_cond","Voltaje condensadora"),("amp_cond","Amperaje condensadora"),
                                                ("temp_sum","Temperatura suministro"),("temp_ret","Temperatura retorno"),
                                                ("temp_amb","Temperatura ambiente"),("lav_filt","Lavado filtros (manj.)"),
                                                ("lav_serp_m","Lavado serpentines (manj.)"),("limp_m","Limpieza int/ext (manj.)"),
                                                ("rub_m","Revisión rubatex (manj.)"),("lav_serp_c","Lavado serpentín (cond.)"),
                                                ("limp_c","Limpieza int/ext (cond.)"),("rub_c","Revisión rubatex (cond.)"),
                                            ],
                                            "portatil": [
                                                ("volt_unit","Voltaje unidad"),("amp_unit","Amperaje unidad"),
                                                ("temp_sum","Temperatura suministro"),("temp_ret","Temperatura retorno"),
                                                ("temp_amb","Temperatura ambiente"),("lav_filt","Lavado de filtros"),
                                                ("limp_unit","Limpieza int/ext"),("rev_dren","Revisión drenaje"),
                                            ],
                                            "ventilador": [
                                                ("volt_unit","Voltaje unidad"),("amp_unit","Amperaje unidad"),
                                                ("limp_unit","Limpieza int/ext"),("rev_fij","Revisión fijaciones"),
                                            ],
                                        }
                                        ...
                                        if True:
                                         ...
                                         if _all_fp:
                                            _fp_pages = []
                                            for _p0 in range(0, len(_all_fp), 6):
                                                _chunk = _all_fp[_p0:_p0+6]
                                                _rh = ""
                                                for _r in range(0, len(_chunk), 2):
                                                    _two = _chunk[_r:_r+2]
                                                    _tds = "".join(
                        ...
                        with st.container():
                            ...
                            with sc1:
                                r_nom_tec  = st.text_input("Nombre técnico",   value=fila_ot.get("Tecnico",""),        key=f"r_nom_tec_{id_ot_sel}")
                                r_superv   = st.text_input("Supervisor",                                               key=f"r_superv_{id_ot_sel}")
                            with sc2:
                                r_nom_cli  = st.text_input("Nombre cliente",   value=fila_ot.get("Nombre_Contacto",""),key=f"r_nom_cli_{id_ot_sel}")
                                r_fec_firma= st.text_input("Fecha",            value=fila_ot.get("Fecha_Ejecucion",""),key=f"r_fec_{id_ot_sel}")

                            # ── Fotos del trabajo ─────────────────────────────────────────────
                            st.divider()
                            _gps_lat_f, _gps_lon_f = st.session_state.get(_gps_key, (None, None))
                            _fob_key_f = f"fotos_oblig_{id_ot_sel}"
                            _fex_key_f = f"fotos_extra_{id_ot_sel}"
                            if _fob_key_f not in st.session_state: st.session_state[_fob_key_f] = {}
                            if _fex_key_f not in st.session_state: st.session_state[_fex_key_f] = []
                            _fob_d_f = st.session_state[_fob_key_f]
                            _fex_l_f = st.session_state[_fex_key_f]
                            _n_tot_f  = len(_fob_d_f) + len(_fex_l_f)
                            _nok_f    = sum(1 for _k,_ in _items_oblig if _k in _fob_d_f)

                            if _items_oblig:
                                st.markdown(f"**📷 Fotos del trabajo — {id_ot_sel}** — {_n_tot_f}/25 &nbsp;|&nbsp; Obligatorias: {_nok_f}/{len(_items_oblig)}")
                                _gf = st.columns(4)
                                for _gfi, (_gfk, _gfl) in enumerate(_items_oblig):
                                    with _gf[_gfi % 4]:
                                        _gfok = _gfk in _fob_d_f
                                        st.markdown(
                                            f"<div style='font-size:11px;padding:3px 5px;border-radius:4px;"
                                            f"background:{'#d1fae5' if _gfok else '#fee2e2'};"
                                            f"border:1px solid {'#6ee7b7' if _gfok else '#fca5a5'};margin:2px'>"
                                            f"{'✅' if _gfok else '❌'} {_gfl}</div>",
                                            unsafe_allow_html=True)
                            else:
                                st.markdown(f"**📷 Fotos del trabajo — {id_ot_sel}** — {_n_tot_f}/25 &nbsp;|&nbsp; _(correctivo/visita: fotos libres)_")

                            _pend_f = [(_k,_l) for _k,_l in _items_oblig if _k not in _fob_d_f]
                            _opcs_f = (
                                [(f"⚠️ {_l}", _k) for _k,_l in _pend_f]
                                + ([("➕ Foto adicional", "__extra__")] if _n_tot_f < 25 else [])
                            )
                            if not _items_oblig:
                                _opcs_f = [("Tomar foto", "__extra__")] if _n_tot_f < 25 else []
                            if _opcs_f:
                                if _items_oblig:
                                    _lbl_f = [_o[0] for _o in _opcs_f]
                                    _sel_f = st.selectbox("¿Qué ítem vas a fotografiar?",
                                                          options=range(len(_lbl_f)),
                                                          format_func=lambda _i: _lbl_f[_i],
                                                          key=f"sel_foto_{id_ot_sel}")
                                    _skey_f = _opcs_f[_sel_f][1]
                                else:
                                    _skey_f = "__extra__"
                            ...
                            _all_f = (
                                [(_k, _fob_d_f[_k]) for _k,_ in _items_oblig if _k in _fob_d_f]
                                + [("__extra__", _fb) for _fb in _fex_l_f]
                            )
                            if _all_f:
                                _nob_f = len(_fob_d_f)
                                _tcf = st.columns(4)
                                for _tif, (_tkf, _tfbf) in enumerate(_all_f):
                                    with _tcf[_tif % 4]:
                                        _capf = (next((_l for _k,_l in _items_oblig if _k==_tkf), "Extra")
                                                 if _tkf != "__extra__" else "Extra")
                                        st.image(f"data:image/jpeg;base64,{_tfbf}", use_container_width=True, caption=_capf)
                                        if st.button("🗑️", key=f"del_f_{id_ot_sel}_{_tif}", use_container_width=True):
                                            if _tkf == "__extra__":
                                                _ixf = _tif - _nob_f
                                                if 0 <= _ixf < len(st.session_state[_fex_key_f]):
                                                    st.session_state[_fex_key_f].pop(_ixf)
                                            else:
                                                st.session_state[_fob_key_f].pop(_tkf, None)
                                            st.rerun()
                            st.divider()

                            generar = st.button("✅ Finalizar y Guardar", type="primary",
                                                use_container_width=True, key=f"generar_{id_ot_sel}")
                            ...
                            if generar:
                                ...
                                if _campos_vacios:
                                    ...
                                    # Las fotos obligatorias se toman en fase 2
    ...
    <table style="table-layout:fixed"><tr>
      <th class="ck" style="width:12pt"></th><th style="width:33%">UNIDAD MANEJADORA</th>
      <th class="ck" style="width:12pt"></th><th style="width:33%">UNIDAD CONDENSADORA</th>
      <th class="ck" style="width:12pt"></th><th>MOTORES Y VENTILADORES</th>
    </tr>
    {''.join(f"<tr><td class='ck'>{ck(v)}</td><td style='border-right:2pt solid #dc2626;overflow:hidden'>{k}</td>" +
             (f"<td class='ck'>{ck(list(ck_co.values())[i])}</td><td style='border-right:2pt solid #dc2626;overflow:hidden'>{list(ck_co.keys())[i]}</td>" if i < len(ck_co) else "<td></td><td style='border-right:2pt solid #dc2626'></td>") +
             (f"<td class='ck'>{ck(list(ck_vent.values())[i])}</td><td style='overflow:hidden'>{list(ck_vent.keys())[i]}</td></tr>" if i < len(ck_vent) else "<td></td><td></td></tr>")
             for i,(k,v) in enumerate(ck_ev.items()))}
    </table>

    <table style="table-layout:fixed"><tr>
      <th class="ck" style="width:12pt"></th><th style="width:58%">TUBERÍA REFRIGERACIÓN Y DESAGÜE</th>
      <th class="ck" style="width:12pt"></th><th>DUCTOS Y REJILLAS</th>
    </tr>
    {''.join(f"<tr><td class='ck'>{ck(v)}</td><td style='border-right:2pt solid #dc2626;overflow:hidden'>{k}</td>" +
             (f"<td class='ck'>{ck(list(ck_duc.values())[i])}</td><td style='overflow:hidden'>{list(ck_duc.keys())[i]}</td></tr>" if i < len(ck_duc) else "<td></td><td></td></tr>")
    ...
    <tr>
      <th style="width:24%">TÉCNICOS</th>
      <th>CONCEPTO</th>
      <th style="width:9%;text-align:center">PESO</th>
      <th style="width:11%;text-align:center">PUNTAJE</th>
      <th style="width:30%">OBSERVACIONES DEL SERVICIO</th>
    </tr>
    <tr>
      <td rowspan="5" style="vertical-align:middle;text-align:center;word-break:break-word">{r_nom_tec}</td>
      <td>Experiencia de los Técnicos</td><td style="text-align:center">20</td><td style="text-align:center"><!--ENC_EXP--></td>
      <td rowspan="5" style="vertical-align:top;word-break:break-word"><!--ENC_OBS--></td>
    </tr>
    ...
    <tr><td colspan="2"><b>TOTAL</b></td><td style="text-align:center"><b>100</b></td><td style="text-align:center"><b><!--ENC_TOT--></b></td><td></td></tr>
    </table>
    ...
    {_fotos_html}
                                    ...
                                    # Guardar — fotos se toman en fase 2
                                    if not _fotos_faltantes_subm:
                                        ...
                                        st.session_state[f"hvac_cat_{id_ot_sel}"]  = _cat_foto_ac
                                        st.rerun()
